...
MA_PLAYER = os.environ.get("MA_PLAYER", "media_player.living_room_satellite_media_player_2")
...
async def play_music(query: str) -> str:
    """
    Search and play music on the living room speakers via Music Assistant.
    ...
    """
    await _ha_call("music_assistant", "play_media", {
        "entity_id": MA_PLAYER,
        "media_id": query,
    })
    return f"Playing {query} on the living room speakers"


@mcp.tool()
async def pause_music() -> str:
    """Pause music."""
    await _ha_call("media_player", "media_pause", {"entity_id": MA_PLAYER})
    return "Music paused"
...
async def resume_music() -> str:
    """Resume music."""
    await _ha_call("media_player", "media_play", {"entity_id": MA_PLAYER})
